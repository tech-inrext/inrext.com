              ...
              {visionaries.map((member) => (
                <div key={member._id} className="px-[0.6rem]">
                  <Link href={`/team/${encodeURIComponent(member.name)}`}>
                    <div className={`h-full flex flex-col justify-center items-center px-5 rounded-xl group cursor-pointer ${isDarkMode ? " border border-gray-500/30" : "border-2 border-blue-500"}`}>
